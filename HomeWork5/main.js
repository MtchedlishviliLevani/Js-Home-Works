...
function AvrgAge(data) {
  const agesArray = data.map((user) => user.age);
  const sumArray = agesArray.reduce((sum, current) => sum + current, 0);
  const avrgAge = sumArray / agesArray.length;
  return avrgAge;
}
console.log(AvrgAge(data));
...
function moreThan40Yo(data) {
  const firstUser = data.find((user) => user.age > 40);
  return firstUser;
}
console.log(moreThan40Yo(data));

// problem 6   ---
// დაწერეთ ფუნქცია რომელსაც გადავცემთ 2 პარამეტრს. მასივს და ელემენტს რომლის წაშლაც გვინდა მასივიდან
// თუ მოცემული ელემენტი იქნება გადაცემულ მასივში მაშინ
// დავაბრუნებთ ახალ მასივს სადაც უკვე ამოშლილი იქნება გადაცემული პარამეტრი

// Problem 7   +++++
//დაწერეთ ფუნქცია რომელიც დაადგენს პარამეტრად გადაცემული სტრინგი არის თუ არა პალინდრომი
function palendrom(word) {
  word = word.toUpperCase();
  if (word === word.toUpperCase().split("").reverse().join("")) {
    return "პალინდრომია";
  } else {
    return "არ არის პალინდრომი";
  }
}
console.log(palendrom("kayak"));
